...
이 글은 Szegedy et al.(2015)의 _Rethinking the Inception Architecture for Computer Vision_ 을 요약이 아니라, 원 논문이 실제로 전개하는 흐름(Introduction → General Design Principles → Factorizing Convolutions → Auxiliary Classifiers → Efficient Grid Reduction → Inception-v2/v3 구성 → Label Smoothing → Training → Low-resolution input 실험 → Experimental Results → Conclusions)을 최대한 그대로 따라가며 한국어로 촘촘히 재서술한 상세 리뷰다.
...
### 🔹 AlexNet 이후의 큰 흐름: 더 깊고 더 넓게, 그리고 효율
...
## 2️⃣ General Design Principles

### 🔹 원칙 1: 표현 병목을 피하라(특히 초반)
...
- 하지만 차원은 대략적인 정보량의 상한/감각을 주므로, 과도한 차원 축소는 위험 신호다.
- 일반적으로 입력에서 출력으로 갈수록 표현 크기가 완만하게 감소해야 하며, 마지막 task representation으로 갈 때만 충분히 줄어들면 좋다.

이 원칙은 뒤에서 grid size를 줄일 때의 블록 설계(Fig. 9, Fig. 10)로 직결된다.

### 🔸 원칙 2: 고차원 표현은 로컬 처리가 쉽다
...
Inception-v3에서 coarsest grid(8×8)에서 채널을 크게 늘리는 설계(Fig. 7)가 이 원칙과 연결된다.

### 🔹 원칙 3: 공간 집계는 낮은 차원 임베딩 위에서도 된다
...
### 🔸 원칙 4: depth와 width를 균형 있게 늘려라
고정된 연산 예산에서 성능을 올리려면, width만 키우거나 depth만 키우는 것보다 둘을 함께 늘리는 것이 더 낫다고 논문은 주장한다. 즉 예산 분배 관점에서 stage별 필터 수와 네트워크 깊이를 균형 있게 조절해야 한다는 것이다.

논문은 이 원칙들이 그럴듯하지만, 실제로는 애매한 상황에서만 지침으로 쓰는 것이 좋다고 말한다. 즉 원칙은 절대 법칙이 아니라, 설계 과정에서 문제 상황을 감지하고 수정하는 기준에 가깝다.

---

## 3️⃣ Factorizing Convolutions with Large Filter Size

### 🔹 왜 factorization이 중요한가: Inception은 fully convolutional이므로
...
### 🔸 3.1 Factorization into smaller convolutions: 5×5를 3×3 두 번으로
...
(Fig. 1: 5×5 conv를 대체하는 미니 네트워크 도식)
...
### 🔹 3.2 Spatial factorization into asymmetric convolutions: 3×3를 3×1+1×3으로
논문은 3×3도 2×2 두 번으로 줄일 수 있겠지만, 더 좋은 방법이 있다고 말한다. 바로 asymmetric conv를 쓰는 것이다.
...
(Fig. 3: 3×3을 대체하는 미니 네트워크, 3×1 conv 포함)

이 아이디어를 일반화하면, n×n conv를 1×n + n×1로 바꿀 수 있다. n이 커질수록 절감 폭도 커진다.

(Fig. 6: n×n conv를 1×n과 n×1로 factorize한 Inception 모듈)

논문은 이 factorization이 초반 레이어에서는 잘 안 먹히지만, 중간 grid size(약 12~20)에서는 매우 잘 동작했으며, 구체적으로 17×17 grid에서는 1×7 + 7×1이 좋은 결과를 줬다고 말한다.

이 부분은 Lucid 구현의 Inception-v3가 17×17 스테이지에서 (1,7), (7,1) 컨볼루션을 반복 사용하는 구조와 직접 대응된다.

---

## 4️⃣ Utility of Auxiliary Classifiers
...
### 🔸 보조 분류기는 regularizer다: BN과 dropout이 핵심
...
(Fig. 8: 17×17 layer 위의 auxiliary classifier, side head BN의 효과)

이 관찰은 BN이 regularizer 역할도 한다는 약한 지지(weak evidence)로 연결된다.

---

## 5️⃣ Efficient Grid Size Reduction

### 🔹 grid 줄이기에서 생기는 계산/표현 병목 문제
전통적으로 feature map의 grid size(H×W)를 줄일 때 pooling을 사용한다. 하지만 표현 병목을 피하려면 pooling 전에 채널 수를 늘리는(expand) 것이 일반적이다. 논문은 이를 정량적으로 설명한다.

d×d grid에서 k 채널을 갖는 feature map을, d/2×d/2 grid에서 2k 채널로 만들고 싶다고 하자. 병목 없이 가려면 대략 다음이 필요하다.

1) stride 1 conv로 2k 채널을 먼저 만들고
2) pooling으로 d/2×d/2로 줄인다

그러면 계산은 큰 grid에서 2k 채널을 만드는 conv가 지배하고, 대략 $2d^2k^2$ 형태로 비싸진다.
...
### 🔸 Fig. 9의 대비: 싸지만 병목 vs 비싸지만 병목 없음
논문은 grid reduction의 두 가지 나이브 대안을 Fig. 9로 보여준다.

(Fig. 9: grid size를 줄이는 두 대안, 좌측은 병목, 우측은 3배 비싼 설계)

즉 병목을 피하려고 하면 계산이 크게 증가하고, 계산을 줄이면 병목이 생기는 딜레마가 있다.

### 🔹 Fig. 10의 해결: 병렬 stride 2 경로 + concat으로 병목을 피한다
...
(Fig. 10: grid size reduction Inception 모듈, 병목 없이 싸게 줄이기)

핵심은 grid가 줄어드는 순간에 채널을 충분히 늘리되, 그 증가를 한 번의 비싼 conv로 하지 않고 병렬 경로로 분산시켜 계산 효율을 유지한다는 것이다.

이 아이디어는 Lucid 구현에서 `_InceptionReduce_V2A`, `_InceptionReduce_V2B` 같은 reduction 모듈로 구현되어 있으며, 실제로 35×35→17×17, 17×17→8×8 전환 구간에서 사용된다.

---

## 6️⃣ Inception-v2 / Inception-v3

### 🔹 Table 1: 전체 네트워크 개요(입력 299×299)
논문은 앞에서 소개한 요소들을 모아 새로운 네트워크를 제안한다. 전체 아키텍처 개요는 (Table 1)에 제시된다.

(Table 1: 제안 네트워크 아키텍처 개요)
...
### 🔸 용어 정리: Inception-v2와 Inception-v3의 관계
...
## 7️⃣ Model Regularization via Label Smoothing

### 🔹 기본 설정: softmax 확률, 크로스 엔트로피, gradient
라벨 스무딩을 설명하기 위해 논문은 먼저 표기를 정리한다.
...
$$
가 되어 gradient가 -1과 1 사이로 bounded된다고 말한다.

### 🔸 원-핫 정답이 만드는 문제: 과도한 확신과 일반화 저하
단일 정답 라벨 $y$를 갖는 분류에서는 보통
$$
q(y)=1,\;\;\; q(k)=0\;\;(k\neq y)
$$
인 원-핫 분포를 쓴다. 이때 크로스 엔트로피를 최소화하는 방향은 $z_y$가 다른 로짓보다 훨씬 커지도록 만드는 것이고, 이 과정에서 모델이 지나치게 확신하게 될 수 있다.

논문은 이로 인해 두 가지 문제가 생길 수 있다고 말한다.

1) 과적합: 학습 데이터에 대해 정답 확률을 1로 만들었다고 일반화가 보장되진 않는다.  
2) 적응성 저하: 가장 큰 로짓과 나머지 로짓들의 차이가 커지면, bounded gradient 특성과 결합되어 모델이 덜 잘 적응할 수 있다.

### 🔹 라벨 스무딩 정의: 원-핫 분포를 prior와 섞는다
논문은 고정 분포 $u(k)$와 스무딩 파라미터 $\varepsilon$를 정의하고, 정답 분포를 다음처럼 바꾼다.
...
ILSVRC 2012에서는 $K=1000$, $\varepsilon=0.1$을 사용했고, top-1/top-5 error가 약 0.2%p 개선되었다고 보고한다(Table 3 참조).

### 🔸 손실의 또 다른 해석: 두 개의 크로스 엔트로피로 분해된다
...
를 가중합으로 함께 최소화하는 것과 같다. $H(u,p)$는 $D_{KL}(u\Vert p)+H(u)$로도 쓸 수 있으며($H(u)$는 상수), 결국 예측 분포 $p$가 prior에서 지나치게 멀어지지 않도록 제약을 거는 형태의 regularization으로 이해할 수 있다.
...
## 8️⃣ Training Methodology

### 🔹 분산 학습 설정: TensorFlow, 50 replicas, 100 epochs
...
### 🔸 Optimizer: momentum에서 RMSProp으로, 그리고 gradient clipping
...
## 9️⃣ Performance on Lower Resolution Input

### 🔹 문제 설정: 작은 객체 패치에선 저해상도 입력이 흔하다
논문은 Multibox, R-CNN 같은 검출 파이프라인에서 post-classification을 할 때, 실제 입력이 전체 이미지가 아니라 객체 주변의 작은 패치인 경우가 많고, 그 객체는 작고 저해상도일 수 있다고 말한다. 그래서 입력 해상도가 낮을 때도 고품질 분류가 가능한지, 그리고 해상도를 올리는 게 정말 이득인지가 질문이 된다.

### 🔸 공정 비교의 핵심: 입력 해상도를 낮추면 계산도 같이 줄어드는 문제
입력을 낮은 해상도로 바꾸면, 같은 네트워크라도 계산이 크게 줄어든다. 그러면 더 어려운 문제를 더 싼 모델로 푸는 셈이 되어 비교가 불공정해진다. 논문은 그래서 계산 비용을 최대한 비슷하게 맞추는 설정을 만든다.

논문은 다음 3가지 설정을 비교한다.

1) receptive field 299×299, stride 2, 첫 레이어 뒤 max-pooling 포함  
2) receptive field 151×151, stride 1, 첫 레이어 뒤 max-pooling 포함  
3) receptive field 79×79, stride 1, 첫 pooling 제거  

세 모델의 계산 비용은 거의 같다고 말한다.

(Table 2: receptive field 크기별 single frame top-1 accuracy)

| Receptive Field | Top-1 Accuracy |
|---|---:|
| 79×79 | 75.2% |
| 151×151 | 76.4% |
| 299×299 | 76.6% |
...
- 입력 해상도가 높을수록 성능이 좋아지긴 하지만, 계산 비용을 맞추면 그 차이는 생각보다 크지 않다.
- 79×79에서도 꽤 높은 정확도가 나온다.

즉 작은 객체 분류 같은 상황에서는 dedicated high-cost low-resolution network라는 아이디어도 고려할 수 있다고 논문은 제안한다.

---

## 🔟 Experimental Results and Comparisons

### 🔹 Table 3: single crop에서의 누적 개선과 Inception-v3의 정의
논문은 Table 3에서 누적 개선을 정리한다. 핵심은 다음이다.

- 각 줄은 이전 변경 사항을 포함한 누적 결과다.
- 마지막 줄의 모든 변경을 포함한 모델을 이후 Inception-v3로 부른다고 말한다.

(Table 3: single crop(단일 크롭) 누적 실험 결과)
...
### 🔹 결론 요지: 원칙 기반 스케일업과 factorization/regularization의 결합
논문은 다음을 결론으로 정리한다.
...
또한 낮은 해상도의 receptive field에서도 좋은 결과가 가능하다는 점(Table 2)을 근거로, 작은 객체 분류 같은 문제에 응용 가능성을 언급한다.
...
2) 병목 회피를 구조로 해결  
grid reduction에서 생기는 병목 문제를 Fig. 10 같은 병렬 블록으로 해결하면서, 네트워크 전체에서 표현 크기가 급격히 줄지 않게 만든다. 이는 이후 많은 아키텍처에서도 반복되는 설계 감각이다.
...
- 본문에서 아키텍처의 세부 채널 구성은 supplementary(model.txt)에 의존하는 부분이 있고, 논문 자체만으로 완전 재현을 하려면 추가 자료를 함께 봐야 한다.

그럼에도 이 논문은 Inception-v3라는 강력한 베이스라인을 제시하면서, 이후의 많은 모델 설계가 factorization, 병목 회피, 규제 기법 같은 키워드를 중심으로 전개되게 만든 중요한 기준점이다.

---

## 👨🏻‍💻 Inception-v3 Lucid 구현
이 파트에서는 `lucid/models/imgclf/inception.py` 안의 Inception-v3 구현을 논문 관점으로 읽는다. Lucid에서는 Inception-v3를 `inception_v3`라는 팩토리 함수로 노출하며, 실제 모델 클래스는 `Inception_V3`다.
...
### 0️⃣ 사전 설정 및 준비 단계
이제부터는 파일 전체를 한 번에 던져놓고 읽기보다, 가장 베이스가 되는 클래스부터 시작해서, 소제목마다 구현 요소 하나씩 맡아서 논문과 연결해 해설한다. 코드 블록은 각 구현 요소의 실제 코드를 그대로 옮기되, 필요한 부분만 그때그때 제시한다.

### 1️⃣ 베이스 클래스: `Inception`
Lucid에서 Inception 계열은 공통적으로 `Inception(nn.Module)`을 베이스로 두고, `num_classes`와 `use_aux` 같은 설정을 저장한다. `forward`는 타입만 열어두고(튜플 반환), 실제 계산은 하위 클래스(v1/v3/v4)가 구현한다.
...
이렇게 베이스를 분리해두면, v1/v3/v4가 공통 파라미터를 공유하면서도 forward 반환 형태(보조 헤드 포함 여부)를 유연하게 바꿀 수 있다.

### 2️⃣ 보조 분류기: `_InceptionAux`
논문 (Sect. 4)은 auxiliary classifier를 최적화 가속이 아니라 regularizer 관점에서 재해석한다. 그리고 side head에 BN이나 dropout을 주면 성능이 좋아졌다고 말한다.

(Fig. 8: 17×17 layer 위의 auxiliary classifier 도식 및 side head에서의 BN 효과)
...
논문은 5×5를 3×3 두 번으로 대체하는 factorization을 설명하며(Fig. 1), 그 구조를 Inception 모듈에 반영한 형태를 제시한다(Fig. 5).

(Fig. 1: 5×5 conv를 3×3 conv 두 번으로 대체하는 미니 네트워크)
(Fig. 5: 5×5가 3×3 두 층으로 치환된 Inception 모듈)
...
(Fig. 10: 병목 없이 grid size를 줄이는 reduction Inception 모듈)
...
논문 (Sect. 3.2)은 n×n을 1×n + n×1로 factorize하는 asymmetric convolution을 제안하고, 17×17 구간에서 1×7과 7×1이 잘 동작했다고 말한다.

(Fig. 6: n×n factorization이 반영된 Inception 모듈, 17×17 grid에서 n=7 선택)
...
(Fig. 7: coarsest 8×8 grid에서 filter bank를 확장한 Inception 모듈)
(Fig. 3: 3×3 conv를 3×1과 1×3으로 factorize한 미니 네트워크)
...
Inception-v3 논문은 Inception-v1의 방향을 유지하면서, 계산 효율을 무너뜨리지 않는 스케일업을 위해 큰 커널을 작은 커널 조합으로 factorize하고, grid reduction에서의 병목을 병렬 경로 concat으로 해결하며, auxiliary classifier와 label smoothing 같은 regularization을 체계적으로 붙여 성능을 끌어올린다. 특히 5×5를 3×3 두 번으로, 7×7을 1×7과 7×1로, 3×3을 3×1과 1×3으로 분해하는 설계는 단순 절감이 아니라 비선형과 BN을 활용해 표현력을 유지하면서 계산을 줄이는 구조적 아이디어로 제시된다. Lucid의 `lucid/models/imgclf/inception.py`에 구현된 `Inception_V3`는 이러한 아이디어를 실제 아키텍처로 조직한 형태이며, 35×35 스테이지에서의 3×3 반복, 17×17 스테이지에서의 1×7/7×1 factorization, reduction 블록의 병렬 경로, 그리고 global average pooling 기반 분류기까지 Inception-v3의 핵심 흐름을 코드로 그대로 따라갈 수 있다.

#### 📄 출처
Szegedy, Christian, et al. "Rethinking the Inception Architecture for Computer Vision." *Proceedings of the IEEE Conference on Computer Vision and Pattern Recognition (CVPR)*, 2016. arXiv:1512.00567. https://arxiv.org/abs/1512.00567.
