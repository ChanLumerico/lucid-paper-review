...
AlexNet이 유명한 이유는 단순히 "큰 CNN을 돌렸더니 성능이 좋았다"가 아니다. 논문은 당시의 컴퓨터 비전 학습 관행에서 

1. 비포화 활성화(ReLU)
2. GPU 기반 고효율 합성곱 구현
3. 데이터 증강과 dropout 중심의 과적합 억제
4. 대규모 데이터셋(ImageNet)에서의 끝단 성능

을 하나의 설계로 묶어 "현실적으로 큰 모델을 끝까지 학습시키는 방법"을 제시한다. 특히 ILSVRC-2010/2012에서 **top-5 error를 큰 폭으로 낮춘** 것이 이 논문의 정량적 임팩트다.

**2012년 당시**의 주류 접근은

1. SIFT 같은 로컬 특징과 
2. BoVW/Spatial Pyramid,
3. Fisher Vector 같은 인코딩/풀링을 조합한 뒤
4. 선형 분류기(SVM 등)를 얹는 방식이 강했다. 

즉 "특징을 사람이 설계하고, 분류기는 상대적으로 단순하게" 가는 흐름이었다. AlexNet은 그 반대로, **모델(특징 추출기) 자체를 거대하게 만들고 end-to-end로 학습**한다. 이 선택은 단순한 구조 변화가 아니라 "학습이 실제로 돌아가느냐(시간/메모리) + 과적합이 터지지 않느냐 + 수렴이 빠르냐"를 동시에 해결해야 하는 시스템 문제로 이어진다. 논문이 ReLU/GPU/증강/dropout을 한 덩어리로 묶어 설명하는 이유가 여기 있다.

---

## 1️⃣ 시대적 배경과 논문의 방향성

### 🔹 대규모 데이터·고해상도 이미지에서의 일반화
...
- 다만 이를 가능하게 하려면, 계산량과 메모리를 감당할 수 있어야 하고, 과적합을 억제할 수 있어야 하며, 학습이 수렴할 만큼 최적화가 잘 되어야 한다.

여기서 "자동으로 학습"은 단순히 편의성 문제가 아니다. 데이터가 120만 장이고 클래스가 1000개면, 모델이 학습 과정에서 마주치는 변형(시점/조명/배경/포즈)이 너무 다양해서, 사람이 만든 특징만으로 일반화를 담보하기가 어렵다. 
...
라는 귀납편향을 통해 "이미지에 맞는 가정"을 내장하고, 여기에 **충분한 용량(큰 모델)** 과 **충분한 데이터**를 결합하면 성능이 크게 뛰는 걸 보여준다.
...
논문은 이미지 입력을 네트워크에 넣을 때 "한 장을 그대로" 넣지 않는다. 데이터 증강과 테스트-time averaging을 위해 **크롭(crop)** 을 적극적으로 활용한다.
...
2. **평가도 ‘한 번’이 아니다.** 테스트에서 10-crop(코너 4 + 중앙 1, 그리고 각각 좌우반전) 예측을 평균한다. 이 방식은 모델 하나로도 작은 앙상블처럼 동작하게 만든다. 뒤에서 보겠지만, 이 10-crop 평균 유무는 top-1/top-5 수치에 꽤 직접적으로 반영된다.

---

## 3️⃣ AlexNet의 설계

논문의 3장은 AlexNet의 핵심이다. 단순히 "층을 쌓았다"가 아니라, **큰 모델을 학습 가능하게 만드는 설계 요소**들을 하나씩 분해해 설명한다. 이 섹션을 읽을 때는 "각 요소가 수렴 속도, 일반화, 그리고 계산 효율에 어떤 영향을 주는가"를 계속 따라가는 게 좋다.
...
<p align="center">
  <img src="https://velog.velcdn.com/images/lumerico284/post/a6a6c932-37ae-4436-98d3-5a1cf1c2fef7/image.png" width="70%">
</p>
...
### 🔹 Multi-GPU와 Overlapping Pooling
...
이 분할은 메모리 제약을 넘기기 위한 실용적 설계이며, 동시에 부분 연결(partial connectivity)을 만들어 모델의 대칭성을 깨는 효과도 갖는다.
...
논문은 ReLU를 쓰면 활성값이 커질 수 있고, **인접 채널 간 경쟁을 유도**해 일반화에 도움을 주기 위해 **LRN**을 도입한다. LRN은 같은 공간 위치 $(x, y)$에서 채널 방향으로 주변 activation의 제곱합을 이용해 정규화한다.

논문 표기대로, $a_{x,y}^i$를 (정규화 전) 채널 $i$의 activation이라고 하면, 정규화 후 $b_{x,y}^i$는
$$
b_{x,y}^i=
a_{x,y}^i\cdot\left(k + \alpha \sum_{j=\max(0, i-n/2)}^{\min(N-1, i+n/2)} (a_{x,y}^j)^2\right)^{-\beta}
$$
여기서
...
논문은 이 상수들을 validation으로 튜닝했고, 사용한 값이 **$k=2, n=5, \alpha=10^{-4}, \beta=0.75$** 라고 명시한다.
...
이제 전체 네트워크를 층 단위로 정리한다.

<p align="center">
  <img src="https://velog.velcdn.com/images/lumerico284/post/c6d5b863-c4e3-4950-8517-5758cd2a3142/image.png" width="80%">
</p>
...
이 구조는 지금 관점에서는 "기본 CNN 스택"처럼 보일 수 있지만, 당시엔

1. ReLU 채택,
2. 대규모 입력/대규모 클래스,
3. 큰 FC 층(4096)과 dropout,
4. GPU 분할까지 포함한 "학습 가능한 규모"가 중요한 포인트였다.

여기서 "큰 FC 층"이 왜 그렇게 강조되는지도 짚고 넘어가자. 논문은 전체 파라미터가 약 **60M**이라고 말하는데, 이 중 상당수가 첫 번째 FC(보통 FC6)에 몰린다. 즉 과적합 위험이 가장 큰 구간도 FC이며, 그래서 dropout이 "FC 앞단에" 들어간다.

또한 Fig. 2의 입력이 224×224인 이유를 논문은 각주로 명시한다. 바로 **256×256으로 리사이즈한 원본에서 224×224 패치를 뽑기 때문**이다. 이처럼 AlexNet의 공간 크기 흐름은 "입력 전처리(리사이즈/크롭) ↔ 아키텍처(스트라이드/풀링)"가 맞물려서 결정된다. 현대 구현들이 첫 conv에 padding을 두거나 adaptive pooling을 쓰는 이유도 결국 이 "크기 맞추기" 문제를 더 유연하게 처리하려는 선택으로 이해할 수 있다.
...
논문은 모델이 큰 만큼 과적합을 강하게 경계한다. 특히 FC 층이 파라미터 수 대부분을 차지하므로, 정규화/증강이 없으면 training error는 내려가도 test error가 크게 나빠질 수 있다. AlexNet은 과적합 억제를 크게 두 축으로 해결하는데 바로 **데이터 증강**과 **dropout**이다.
...
#### 광학적 증강: RGB 채널의 조명 변화(색상 jitter)
...
### 🔹 Dropout: co-adaptation을 막는 확률적 정규화
...
- dropout 확률은 **0.5**이고, **첫 두 FC 층(FC6/FC7)에만 적용**한다. conv 쪽에는 적용하지 않는다.  
...
이 섹션은 AlexNet이 단순히 구조가 좋아서가 아니라, **학습 설정 자체가 대규모 학습을 가능하게 했음**을 보여주는 파트다. 구현/재현 관점에서 중요하므로, 변수들을 정리해가며 읽자.
...
즉 momentum, weight decay, 그리고 미니배치 $D_i$에서의 평균 그래디언트를 모두 섞어 한 번에 업데이트한다. 흥미로운 포인트는 논문이 "이 정도의 작은 weight decay가 단지 regularizer가 아니라 training error를 낮추는 데에도 중요했다"고 말한다는 점이다. 즉 수치적으로 더 안정적인 최적화 경로를 제공하는 효과가 있었다고 이해할 수 있다.

초기화/스케줄도 비교적 구체적으로 적는다.

- **가중치**: 평균 0, 표준편차 0.01의 Gaussian에서 샘플링  
...
이 디테일들은 큰 모델을 학습시키는 데 어떤 수치적/시스템적 선택이 필요했는지를 보여주는, 당시로서는 매우 실전적인 레시피로 읽힌다.
...
이 의사코드는 기본적인 뼈대만 보여준다. AlexNet의 실제 기여는 여기에 ReLU로 빠른 수렴, dropout으로 FC 과적합 억제, 그리고 GPU 최적화로 학습 시간을 현실화하는 요소들이 결합된다는 점이다.
...
<p align="center">
  <img src="https://velog.velcdn.com/images/lumerico284/post/4f8cccf7-0f49-46d2-9fe9-85808626c518/image.png" width="50%">
</p>
...
<p align="center">
  <img src="https://velog.velcdn.com/images/lumerico284/post/9b50a565-3a89-49f0-bc51-97ebe60f3183/image.png" width="60%">
</p>
...
또 하나의 실무적인 디테일로, 논문은 4.1에서 말한 **10-crop 평균을 하지 않았을 때** ILSVRC-2010에서 top-1/top-5 error가 **39.0%/18.3%** 라고 각주로 덧붙인다.
...
<p align="center">
  <img src="https://velog.velcdn.com/images/lumerico284/post/afdf66d6-a6ce-4b9f-9b79-bbf46e7e2f85/image.png" width="50%">
</p>
...
<p align="center">
  <img src="https://velog.velcdn.com/images/lumerico284/post/08f19c22-2a37-4d4e-84af-78e68257c689/image.png" width="70%">
</p>

이 파트의 요지는 "모델이 단순히 훈련 데이터를 암기한 것이 아니라, 일반화 가능한 표현 공간을 학습했다"는 정성적 근거를 보이려는 것이다.

논문은 Fig. 4의 왼쪽 패널에서, 객체가 중심에 있지 않거나(off-center), 배경이 복잡해도 top-5 후보들이 꽤 그럴듯하게 좁혀지는 사례를 보여준다. 오른쪽 패널의 최근접 이웃 예시는 마지막 은닉층(4096차원 표현)이 "유사한 시각적/의미적 특징"을 보존하는 공간으로 학습되었음을 보여주려는 의도다.
...
그리고 이 논문을 한계의 관점으로 읽을 때도 배울 게 많다.
...
그럼에도 AlexNet의 조합 설계는 여전히 역사적으로 중요하다. "큰 CNN을 끝까지 학습시키기 위해 무엇이 동시에 필요했는가(최적화/구현/정규화/증강)"를 논문 단위로 한 번에 묶어 보여준다는 점에서, 이후의 거의 모든 CNN 계열 연구를 이해하는 출발점 역할을 한다고 느꼈다.

---

## 👨🏻‍💻 AlexNet 구현하기

아래는 [`lucid`](https://github.com/ChanLumerico/lucid)에 존재하는 AlexNet 구현을 실제로 읽고, 논문 구조와 1:1로 대응시켜 해설하는 파트다. 여기서는 논문의 "정확히 동일한 구현"을 재현하기보다, Lucid 코드가 논문에서 제시한 설계 요소들을 어떤 방식으로 담고 있는지(또는 생략했는지)를 코드 단위로 확인한다.

### 0️⃣ 구현 위치와 엔트리포인트

Lucid의 AlexNet은 [`alex.py`](https://github.com/ChanLumerico/lucid/blob/main/lucid/models/imgclf/alex.py)에 구현되어 있다.
...
논문에서 강조한 ReLU와 pooling 설계 요소는 Lucid 코드에 명확히 드러난다.

반면 논문에 있던 LRN은 `self.conv` 내부에 등장하지 않는다. 즉 Lucid의 AlexNet은 LRN을 생략한 변형이다.

Lucid 구현의 공간 크기 흐름을 입력 224×224 기준으로 간단히 추적하면 아래처럼 정리할 수 있다.
...
### 2️⃣ 논문과 Lucid 구현의 차이
마지막으로, 논문을 기준으로 Lucid 구현을 읽을 때 없거나 다른 것을 명확히 적어두면 혼동이 줄어든다.
