...
논문 텍스트를 따라 층을 기술하면 대략 다음과 같은 흐름이다(핵심 파라미터 중심).

#### 입력
- 입력 패치: $224 \times 224 \times 3$ (RGB).  
- (논문 주석) Fig. 2의 입력이 224인 이유는, 앞단에서 256 크기로 리사이즈 후 크롭을 사용하기 때문.

#### Conv1
- 커널: $11 \times 11$, stride 4  
- 출력 채널: 96  
- 뒤에 ReLU  
- 뒤에 LRN  
- 뒤에 MaxPool(kernel 3, stride 2)
...
그럼에도 AlexNet의 “조합 설계”는 여전히 역사적으로 중요하며, 큰 모델 학습의 기본 원리를 이해하는 데 좋은 교재가 된다.
